...
| Data/description and status         | `DataTable`, `DescriptionList`, `StatusMarker`, `ContentState`, `InlineAlert`          | Use `DataTable` for headed collections with row selection or column sizing; use `DescriptionList` for one object's facts. Facts and errors must name their source/action.                                             |
