...
On touch-sized viewports, keep header glyphs visually compact inside square 44px hit areas. A playground's stage must show or deliberately scroll its entire example; its inspector should fit the controls it actually contains. If a sample command only demonstrates selection, say so and show the selected value in the input. Omit status metadata that does not change a reader's next action.
